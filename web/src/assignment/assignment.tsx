...
function Assignment() {
  ...
  useEffect(() => {
    ...
    const getAssignment = async () => {
      const response = await fetch(`/api/classroom/assignment/${assignmentId}`);
      if (response.ok) {
        const json = await response.json();
        setAssignmentInfo(json);
      } else {
        setErrorMessage(
          "Error getting the assignment. Either the assignment does not exist or you do not have the permissions to see it."
        );
      }
    };
