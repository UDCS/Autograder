      ...
      if (!loaded) {
        try {
          ...
          if (response.ok) {
            ...
            const loggedIn = json["message"] == "true";
            if (!loggedIn) {
              setIsLoggedIn(false);
              redirectToLogin();
            }
    ...
    <>
      <div id="accountRoot">
        {isLoggedIn ? (
              ...
              <tr>
                ...
                <td colSpan={2} align="left" style={{ paddingRight: "1%" }}>
                  <div className="colorTheme">
                    <ToggleSwitch />
                    <div className="label" style={{ marginLeft: "1%" }}>
                      Dark Mode
                    </div>
                  </div>
                </td>
                <td id="linkSentParent">
                  <div id="linkSent">{passwordMessage}</div>
                </td>
              </tr>
            </table>
            <BlueButton
              id="submit"
              style={submitState == ButtonState.Error ? { color: "red" } : {}}
          ...
          <>
            <h1 className="header" style={{ marginBottom: "4%" }}>
